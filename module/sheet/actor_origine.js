...
import {RaceBonus, MetierBonus} from "../data/constants.js"; // Import de la constante METIERS
...
export default class LiberCharacterSheet extends HandlebarsApplicationMixin(ActorSheetV2) {
  ...
  async _prepareContext() {
    ...
    let competences  =items.filter(item => item.type == "competence");
    let competencesMetier  =await this.getCompetencesMetier(this.actor.system.metier,competences);
    ...
    return {
      ...
      competencesMetier :competencesMetier ,
 ...
 async _onVerif() {
  ...
  if(this.actor.system.level==1){
    ...
    if(race=="Draconien"){total=total+30}
    else if(race=="Humain"){total=total+30}
    else {total=total+60}
...
console.log(metier,race,bonusRace,bonusMetier)
  let updatesActeur = {};

  competencesConnues.forEach(nom => {
    const valeur = parseInt(competences[nom]) || 0;
    // Lire le bonus depuis l'objet (peut être positif ou négatif)

    // +valeur si la compétence est présente dans bonusRace / bonusMetier
    const bonusRaceVal   = nom in bonusRace   ? bonusRace[nom]   : 0;
    const bonusMetierVal = nom in bonusMetier ? bonusMetier[nom] : 0;

    let bonus = bonusRaceVal + bonusMetierVal;
    if(race=="Humain"){bonus=bonus+10}
    // Limites ajustées par le bonus net
    const minValeur = -30 + bonus; // malus élargit le bas
    let maxValeur =  30 + bonus; // bonus élargit le haut
    if(maxValeur>50){maxValeur=50}

    if (valeur < minValeur) {
      console.warn(`⚠️ ${nom} < ${minValeur}, ajustée.`);
      updatesActeur[`system.competences.${nom}`] = minValeur;
    } else if (valeur > maxValeur) {
      console.warn(`⚠️ ${nom} > ${maxValeur}, ajustée.`);
      updatesActeur[`system.competences.${nom}`] = maxValeur;
    }

    //console.log(`Compétence: ${nom} | Valeur: ${valeur} | Bonus: ${bonus > 0 ? "+" : ""}${bonus} | Plage: [${minValeur}, ${maxValeur}]`);
  });
  ...
  await this.actor.update(updatesActeur);
   ...
   async getCompetencesMetier(Metier,competencesActuelles) {
      // Compétences actuelles de l'acteur
     // const items = this.document.items;
     // const competencesActuelles = items.filter(item => item.type === "competence");

      // Map prerequis -> quantity max possédée dans ce groupe
      // Ex: { "Actif": 2, "Passif": 1, "Support": 0 }
      const niveauxParPrerequisMap = new Map();
      for (const c of competencesActuelles) {
          const typePrereq = c.system.prerequis;
          const quantite   = c.system.quantity ?? 1;
          const actuel     = niveauxParPrerequisMap.get(typePrereq) ?? 0;
          // On garde le niveau MAX possédé dans ce groupe
          if (quantite > actuel) niveauxParPrerequisMap.set(typePrereq, quantite);
      }

      // Récupérer le compendium
      ...
      for (const item of docs) {
          // Filtrer par métier
          if (item.system.metier !== Metier) continue;

          const typePrereq  = item.system.prerequis;  // "Actif", "Passif" ou "Support"
          const niveauItem  = item.system.quantity ?? 1;
          const niveauPossede = niveauxParPrerequisMap.get(typePrereq) ?? 0;

          // On veut uniquement le prochain niveau dans ce groupe de prérequis
          if (niveauItem !== niveauPossede + 1) continue;

          // Nettoyer la bio pour le title du <option>
          const biographyRaw = item.system.biography ?? "";
          item.biographyPlain = biographyRaw.replace(/<[^>]*>/g, "");

          competencesDisponibles.push(item);
      }

      return competencesDisponibles;
  }
  ...
  static async #initializerNiveau1(event, target) {
      ...
      competencesConnues.forEach(nom => {
        const bonusRaceVal   = bonusRace[nom]   ?? 0;
        const bonusMetierVal = bonusMetier[nom] ?? 0;
        const bonusTotal     = bonusRaceVal + bonusMetierVal;

        // Valeur de départ = bonus de race/métier
        updatesActeur[`system.competences.${nom}`] = bonusTotal;
      });
  ...
  //#region Actions
    ...
    static async #addCompt(event, target) {
      ...
      // Récupérer l'id sélectionné dans le select
      ...
      // Récupérer la compétence dans le compendium
      const pack = game.packs.get("libersf.competences");
      if (!pack) {
          ui.notifications.error("Compendium libersf.competences introuvable.");
          return;
      }
      
      const itemData = await pack.getDocument(selectedId);
      if (!itemData) {
          ui.notifications.error("Compétence introuvable dans le compendium.");
          return;
      }

      // Vérifier si l'acteur possède déjà une compétence du même prérequis/niveau
      const typePrereq = itemData.system.prerequis;
      const niveauItem = itemData.system.quantity ?? 1;
      const dejaPresent = actor.items.some(i =>
          i.type === "competence" &&
          i.system.prerequis === typePrereq &&
          i.system.quantity === niveauItem
      );
      if (dejaPresent) {
      ...
      // Ajouter la compétence à l'acteur
    ...
    static async #onItemRollDamage(event, target) {
      ...
      let skillMap = {
        ...
        let key = skillMap[compt] || null;

        let maitrise = 0;

        if (key) {
            maitrise = Math.floor(this.actor.system.competences[key] / 5);
        }

        // Bonus racial Elfen
        if (this.actor.system.race === "elfen") {
            maitrise += 2; // +1 étoile = +2 demi-étoiles
        }

        // techno = niveau technologique de l'objet
        // (0=★, 1=★☆, 2=★★, etc.)

        let diff = techno - maitrise;
        if (diff >= 5) {
            ui.notifications.warn(
              "Technologie trop avancée pour être utilisée."
            );
            return;
        }
        let malus = 0;
